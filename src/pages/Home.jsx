...
import { CheckCircle, Clock, Download, Upload, BarChart2, Users } from 'lucide-react'
import { getTurmas, getAlunos, getChamadaByData, exportData, importData } from '../utils/storage'
...
export default function Home({ navigate }) {
  ...
  const loadData = useCallback(() => {
    ...
    t.forEach(turma => {
      ...
      const chamada = getChamadaByData(turma.id, dataAula)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Backup */}
      <div className="border border-gray-200 rounded-xl p-4 bg-white no-print">
        <h3 className="font-semibold text-gray-700 mb-1">Backup de Dados</h3>
        <p className="text-xs text-gray-400 mb-3">
          Os dados ficam salvos neste navegador. Faça backup regularmente para não perder informações.
        </p>
        <div className="flex gap-3 flex-wrap">
          <button
            onClick={exportData}
            className="flex items-center gap-2 px-4 py-2 text-sm bg-slate-700 text-white rounded-lg hover:bg-slate-600 transition"
          >
            <Download size={15} className="text-white" />
            Exportar Backup
          </button>
          <label className="flex items-center gap-2 px-4 py-2 text-sm bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition cursor-pointer">
